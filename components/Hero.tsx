      ...
      <div className="max-w-6xl mx-auto">
        <div className="max-w-3xl">
          ...
          <div className="flex flex-wrap gap-4 mb-10">
            <a href={data.cta1.href} className="px-6 py-3 bg-indigo text-white text-sm font-medium rounded-full hover:bg-indigo-dark transition-colors">
              {data.cta1.label}
            </a>
            <a href={data.cta2.href} className="px-6 py-3 border border-border text-sm font-medium text-navy rounded-full hover:border-navy transition-colors">
              {data.cta2.label}
            </a>
          </div>

          <p className="text-sm text-muted mb-20">
            <span className="text-ink font-medium">{data.byline.name}</span>
            {" · "}
            {data.byline.role}
          </p>
        </div>
        ...
        <div className="border-t border-border pt-10 mb-12">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
            {data.stats.map((stat) => (
              <div key={stat.label}>
                <div className="font-serif text-4xl md:text-5xl text-navy mb-2">{stat.value}</div>
